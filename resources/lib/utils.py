...
def refreshToken():
    ...
    data = requests.post(
        ref_TokenApi, headers=ref_TokenHeads, data=ref_TokenPost
    ).json()
...
def convertEpoch(epoch):
    date = datetime.datetime.fromtimestamp(
        int(epoch) / 1000, datetime.timezone.utc
...
def verifyOTP(number, otp):
    ...
    b64_number = base64.b64encode(number.encode()).decode()
    verify_url = (
        "https://jiotvapi.media.jio.com/userservice/apis/v2/loginotp/verify?langId=6"
    )
    verify_payload = {
        "number": b64_number,
        "otp": otp,
    ...
    with PersistentDict("localdb") as db:
        if not db.get("channelList"):
            channels = requests.get(
                "https://jiotvapi.cdn.jio.com/apis/v3.1/getMobileChannelList/get/?langId=6&os=android&devicetype=phone&usertype=guest&version=389&langId=6",
    ...
    if type == "genre":
        ...
        for i in channelList:
            ...
            if str(i["channelLanguageId"]) not in languages.keys():
                continue
            if (
                languages[str(i["channelLanguageId"])] == query
                and genres[str(i["channelCategoryId"])] == query2
            ):
                finals.append(i)
    return finals
...
def catchup_data(day, id):
    return requests.get(
        f"https://jiotvapi.cdn.jio.com/apis/v1.3/getepg/get?offset={day}&channel_id={id}&langId=6",
        headers={"user-agent": "okhttp/4.11.0"},
    ).json()
